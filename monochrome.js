...
const frontendDir = path.join(__dirname, 'frontend', 'components');
...
walk(frontendDir, (filePath) => {
  if (filePath.endsWith('.tsx') || filePath.endsWith('.ts')) {
    ...
    // 2. Clean up double spaces caused by removing dark classes
    content = content.replace(/\s{2,}/g, ' ');

    // 3. Replace color palette mappings
    const colorMap = {
      ...
      'bg-brand-600': 'bg-[#111111]', // Map main brand bg to black
      ...
      'text-brand-500': 'text-[#444444]', // Icons like map pin
    };
